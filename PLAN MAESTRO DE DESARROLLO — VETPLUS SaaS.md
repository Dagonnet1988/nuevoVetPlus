...
| Fase S | Seguridad urgente (hardening pre-producción) | ⚠️ PARCIAL (base lista, faltan cierres) | 0d857a2 |
| Fase MT1 | Multi-tenancy DB (tenants + tenant_id) | ✅ COMPLETO | 063e60d |
| Fase MT2 | Multi-tenancy Auth (JWT + middleware + frontend) | ⚠️ PARCIAL (propagación incompleta) | 19d49d7 |
| Fase MT3 | RLS — aislamiento de datos por tenant | ⚠️ PARCIAL (SQL completo, adopción app parcial) | 02c3659 |
...
#### Parcial
- ⚠️ Rate limiting implementado pero no aplicado en toda la API (auth/admin/general).
- ⚠️ `AuthGuard` activo en layout principal, pero `change-password` sigue sin guard.
- ⚠️ MT2 implementado en capas base, pero no propagado de extremo a extremo en login/refresh.
- ⚠️ MT3 en SQL completo, adopción en controladores aún incompleta (`query()` global en muchos puntos).

#### Pendiente crítico
- ❌ Incluir `id_tenant` explícito en todos los `INSERT/UPDATE` de tablas core.
- ❌ Migrar controladores a `queryWithTenant()` o patrón transaccional con `SET LOCAL app.tenant_id`.
- ❌ Alinear schema/roles con rutas (`aux_admin`, `aux_vet`, `assistant`).
- ❌ Corregir inconsistencias de consentimiento (`id` vs `id_cliente` / `id_consentimiento`).
...
Estado S.1: ⚠️ PARCIAL (S.1.1 listo; S.1.2 por confirmar/documentar)
...
Estado S.2: ⚠️ PARCIAL (implementado, falta aplicación transversal)
...
Estado S.5: ⚠️ PARCIAL (S.5.1 listo; S.5.2 pendiente)
...
Estado MT2.1: ⚠️ PARCIAL (payload/middleware listos; login/refresh aún requieren propagación completa de `id_tenant`)
...
Estado MT2.2: ⚠️ PARCIAL (middleware creado y montado en varias rutas; falta estandarización completa)
...
Estado MT2.3: ⚠️ PARCIAL (header ya sale del frontend; falta consumo/validación consistente en backend)
...
Estado MT3: ⚠️ PARCIAL (policies SQL completas; falta migrar controladores a patrón tenant-aware de forma sistemática)
