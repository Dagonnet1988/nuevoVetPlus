...
-- ===========================================
-- TABLA DE ARCHIVOS DE HISTORIA CLÍNICA
-- ===========================================

-- Tabla de archivos adjuntos a consultas clínicas
CREATE TABLE clinical.archivos_consulta (
    ...
    descripcion TEXT,
    categoria VARCHAR(50) DEFAULT 'general' CHECK (categoria IN ('radiografia', 'laboratorio', 'receta', 'examen', 'general')),
    fecha_subida TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    subido_por UUID REFERENCES vetplus_auth.usuarios(id_usuario),
    activo BOOLEAN DEFAULT true,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX idx_archivos_consulta_categoria ON clinical.archivos_consulta(categoria);
CREATE INDEX idx_archivos_consulta_fecha_subida ON clinical.archivos_consulta(fecha_subida);
CREATE INDEX idx_archivos_consulta_activo ON clinical.archivos_consulta(activo);

-- Comentarios
COMMENT ON TABLE clinical.archivos_consulta IS 'Archivos adjuntos a consultas clínicas (radiografías, exámenes, etc.)';
COMMENT ON COLUMN clinical.archivos_consulta.categoria IS 'Categoría del archivo: radiografia, laboratorio, receta, examen, general';
